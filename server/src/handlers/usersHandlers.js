...
const postUser = async (req, res) => {
    try {
      let db = getDb()
      const { username, uid, email, image, role, permissions, firstName, lastName, dateOfBirth, phone, password } = req.body;
...
const getConfirmedReservations = async (req, res) => {
    try {
      ...
      const confirmedReservations = user.reservation.filter(reservation =>
        reservation.state === 'confirmed' && reservation.billing_status === 'Accepted'
      );
