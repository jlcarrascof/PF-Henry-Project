import { initMercadoPago, Wallet } from "@mercadopago/sdk-react";
import { useState, useEffect } from "react";
import Types from "./Types.ts";
import "./Pasarela.css";

//! un ejemplo de como iria la ruta, con el id al lado de la roomm
/*
/room/hsajdsd-dasdasd
/room/hsajdsd-dasddas
:)
*/

let Reservas: Types.Pasarela.ReservInfo[] = [],
  PagoTotal: number = 0;

let Executed = false;

export const Pasarela: React.FC = () => {
  const [MP, SetMP] = useState({
    ID: "",
    Succsess: false,
  });

  const [PP, SetPP] = useState({
    link: "",
    Succsess: false,
  });

  useEffect(() => {
    (async () => {
      if (Executed) return;
      Executed = true;
      initMercadoPago("TEST-90634ea0-ef39-4285-ba3b-3f6daf135572");

      const DataFromLS = localStorage.getItem("ReservInfo");
      console.log("Data:", DataFromLS);
      if (!DataFromLS || DataFromLS == "") {
        window.location.replace("/");
        return;
      }
      /* Heren yegar que la paloma que?*/ //localStorage.setItem('ReservInfo', ''); // si se quiere cons los datosos
      let ReservInfo: Types.Pasarela.ReservInfo;
      try {
        ReservInfo = JSON.parse(DataFromLS);
      } catch (Err) {
        alert(
          "Error al obtener datos de habitación: " + (Err as string).toString()
        );
        return;
      }

      Reservas = [ReservInfo];
      //const Reservas = Types;
      PagoTotal = 0;
      Reservas.forEach((R) => (PagoTotal += R.precio * R.dias));

      //const [ID, setID] = useState<string>("null");
      // e.target.setAttribute('Loading', 'Si');
      const orderData: Types.Pasarela.OrderData = [];

      //! aqui hice la conversion de titulos pues
      Reservas.forEach((Reserva) => {
        orderData.push({
          id: Reserva.id,
          unit_price: Reserva.precio,
          quantity: Reserva.dias,
          title: Reserva.titulo,
        });
      });
      console.log("Order:", orderData);

      try {
        const Result = await fetch(
          "http://localhost:3002/payment/create-preference",
          {
            method: "post",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify(orderData),
          }
        );
        let Data = await Result.json();
        SetMP({
          ID: Data.id,
          Succsess: true,
        });
        console.log("MP result", Data);

        const Result2 = await fetch(
          "http://localhost:3002/payment/paypal-order",
          {
            method: "post",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify(orderData),
          }
        );

        let Data2 = await Result2.json();
        console.log("PP result", Data2);
        if (Data2.created == true) {
          SetPP({
            link: Data2.link,
            Succsess: true,
          });
        }
      } catch (error) {
        console.log("MP-PP Error: ", error);
      }
    })();
  });

  return (
    <div className="Reservas">
      {Reservas.map((Reserva) => (
        <div className="PasarelaCard">
          <img className="PasarelaImg" src={Reserva.image} />
          <div className="PasarelaInfo">
            <h2 className="PasarelaTitulo">{Reserva.titulo}</h2>
            <div className="InfoPrecio">
              <div className="InfoReserva">
                <span className="PasarelaDias">Dias: {Reserva.dias}</span>
                <span className="PasarelaPrecio">
                  Precio/dia: {Reserva.precio}
                </span>
              </div>
              <div className="PasarelaTotal">
                <span className="PasarelaPrecio">
                  Total: {Reserva.precio * Reserva.dias}
                </span>
              </div>
            </div>
          </div>
        </div>
      ))}
      <div className="Pagos">
        <div className="PagoDesc">
          {!MP.Succsess && <h2>Error al obtener preferenceID</h2>}
          {!PP.Succsess && (
            <h2>Error al obtener el enlace de pago de paypal</h2>
          )}
        </div>
        <div className="PagosTotal">
          <span className="TotalPagar">Pago total: {PagoTotal}</span>
          {MP.Succsess && <Wallet initialization={{ preferenceId: MP.ID }} />}
          {PP.Succsess && (
            <div className="Paypal-container">
              <a className="PaypalButton" href={PP.link}>
                <img src="data:image/svg+xml;base64," />
              </a>
              <div className="paypal-powered-by">
                <span className="paypal-button-text">Desarrollado por </span>
                <img
                  src="data:image/svg+xml;base64,"
                  className="paypal-logo paypal-logo-paypal paypal-logo-color-blue"
                />
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default Pasarela;
